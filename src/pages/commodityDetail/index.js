...
class CommodityDetail extends Component {
    ...
    render() {
        ...
        const onFinish = () => {
            ...
            formdata.append("roomPicPath1", this.state.file[0]||null);
            formdata.append("roomPicPath2", this.state.file[1]||null);
            formdata.append("roomPicPath3", this.state.file[2]||null);
            formdata.append("roomPicPath4", this.state.file[3]||null);
            axios.put('http://116.85.12.238:8080/aihotel/manager/room',formdata).then((res) => {
            ...
            <Card title="商品修改" extra={
                ...
                >
                    ...
                    <Form.Item
                        label="设施"
                    >
                        <Checkbox.Group options={options} value={this.state.facilities} onChange={onChange} />
                    </Form.Item>
                    <Form.Item
                        label="房型描述"
                    >
    ...
    componentDidMount () {
        if(this.props.roomId != '') {
            axios.get('http://116.85.12.238:8080/aihotel/manager/room/' + this.props.roomId).then((res) => {
                var data = res.data;
                var pic = [{
                        uid: '-1',
                        name: 'image.png',
                        status: 'done',
                        url: 'https://lankeren-1254433704.cos.ap-guangzhou.myqcloud.com' + data.roomPic.roomPicPath1,
                    },
                    {
                        uid: '-2',
                        name: 'image.png',
                        status: 'done',
                        url: 'https://lankeren-1254433704.cos.ap-guangzhou.myqcloud.com' + data.roomPic.roomPicPath2,
                    },
                    {
                        uid: '-3',
                        name: 'image.png',
                        status: 'done',
                        url: 'https://lankeren-1254433704.cos.ap-guangzhou.myqcloud.com' + data.roomPic.roomPicPath3,
                    },
                    {
                        uid: '-4',
                        name: 'image.png',
                        status: 'done',
                        url: 'https://lankeren-1254433704.cos.ap-guangzhou.myqcloud.com' + data.roomPic.roomPicPath4,
                    }];
                const checked = data.roomFacility.map((item, index) => {
                    return item.id;
                });
                this.setState({
                    roomType: data.fullInfo.roomType,
                    price: data.fullInfo.price,
                    roomFloor: data.fullInfo.roomFloor,
                    roomSize: data.fullInfo.size,
                    bedType: data.fullInfo.bedType,
                    bedSize: data.fullInfo.bedSize,
                    people: data.fullInfo.personNum,
                    isBreakfast: data.fullInfo.isBreakfast,
                    isVip: data.fullInfo.isVip,
                    facilities: checked,
                    describe: data.fullInfo.description,
                    fileList: pic,
                });
            }).catch(() => {
                console.log("error");
...
const mapState = (state) => ({
    ...
    roomId: state.getIn(['commodity','commodityDetailId']),
    id: state.getIn(['login','hotelId']),
});


export default connect(mapState, null)(CommodityDetail);